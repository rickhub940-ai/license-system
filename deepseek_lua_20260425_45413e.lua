-- ==================== UI เลือกโหมด (Normal / God) - อยู่เหนือสุด ====================
-- ถ้าไม่เลือกโหมด -> UI จะค้าง -> ไม่รันสคริปต์หลักเด็ดขาด
-- ถ้าเลือกโหมด -> UI หายทันที -> รัน AC BYPASS -> โหลด WindUI -> รัน Dipper HUB เต็มระบบ
...
local titleTween = TweenService:Create(title, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {TextTransparency = 0})
local descTween = TweenService:Create(desc, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {TextTransparency = 0})
...
local function createButton(text, order)
    ...
    local appearBtn = TweenService:Create(button, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
    ...
    button.MouseEnter:Connect(function() tween(Color3.fromRGB(235, 235, 235)) end)
    button.MouseLeave:Connect(function() tween(Color3.fromRGB(255, 255, 255)) end)
    button.MouseButton1Down:Connect(function() tween(Color3.fromRGB(210, 210, 210)) end)
    button.MouseButton1Up:Connect(function() tween(Color3.fromRGB(235, 235, 235)) end)
...
normalBtn.MouseButton1Click:Connect(function()
    if modeSelected then return end
    modeSelected = true
    selectedMode = "Normal"
    print("✅ เลือก Normal Mode - กำลังลบ UI...")
    destroyUI()
...
godBtn.MouseButton1Click:Connect(function()
    ...
    modeSelected = true
    selectedMode = "God"
    print("✅ เลือก God Mode - กำลังลบ UI...")
    destroyUI()
...
-- ==================== ฟังก์ชันโหลดสคริปต์หลัก Dipper HUB เต็มระบบ ====================
local function loadMainScript()
    print("🚀 กำลังโหลด Dipper HUB เต็มระบบ...")
    ...
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if input.KeyCode == Enum.KeyCode.T then toggle() end
    end)
    ...
    print("✅ Dipper HUB โหลดเรียบร้อยแล้ว (โหมด: " .. selectedMode .. ")")
end